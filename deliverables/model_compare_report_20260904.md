...
> 更新：2026-09-04 追加视觉模态实测，确认 `deepseek-v4-pro` 明确不支持图片输入；`glm-5.2` 会接收图片 Token 但结果不可信；`kimi-k3` 当前额度受限。详见文末「视觉模态专项实测」。
...
## 视觉模态专项实测（2026-09-04 追加）

问题：既然 DeepSeek V4 Pro 复杂题最强，能否让它直接做视觉 OCR？
...
| 模型 | 是否支持图片输入 | 实测结果 |
|---|---|---|
| `deepseek-v4-pro` | 官方标注不支持 | HTTP 400，明确报错：`Model do not support image input` |
| `glm-5.2` | 官方标注不支持 | 返回 200，接收了约 2,908 个 prompt Token，但只输出 4 题且 `page_title` 为 null，结果不可信，疑似无法真正理解图片 |
| `sensenova-6.8-flash-lite` | 支持 | 可正常看图并返回结构化 JSON |
| `kimi-k3` | 官方标注支持，仅 Base64 | 连续 3 次返回 429：`inference tpm exhausted`，当前额度不可用 |

### 为什么不能让 DeepSeek V4 Pro 做视觉？

1. 它明确不支持图片输入，请求会被 API 直接拒绝。
2. 它不是“效果差一点”，而是能力边界不同：没有视觉编码器，无法读取作业图片。
3. 即使绕过限制也没意义：它拿不到图像内容，只能凭文字猜测，反而会编造题目。

### GLM-5.2 为什么更不能用于视觉？

它比 DeepSeek 更危险：

- DeepSeek 会直接报错，至少不会污染结果；
- GLM-5.2 会接收图片 Token 并返回看似正常的回答；
- 本次测试中它只返回 4 题，且结构不完整，属于“看似能用，实际不可信”。

这类模型如果接入视觉链路，会造成隐性错误，比直接报错更麻烦。
...
```text
第一步：SenseNova 6.8 看图识别题目和学生答案
第二步：DeepSeek V4 Pro 基于识别结果生成标准答案和解析
```

这不是为了省 Token，而是模型能力边界决定的硬约束。
