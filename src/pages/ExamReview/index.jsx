...
export default function ExamReview({ task, onClose, onSave }) {
  ...
  // 图片实际渲染尺寸（Contain 缩放后的像素值，用于 OCR 框定位）
  ...
  useEffect(() => {
    ...
    // 计算留白偏移（图片在容器中居中后的偏移量）
    const padLeft = (containerW - rw) / 2
    const padTop = (availH - rh) / 2
