...
/**
 * 渲染完整 HTML（已含 KaTeX DOM + 二维码 + 字体 data URL）。
 * 与 PrintPreview 完全相同的渲染流程，保证 100% 视觉一致。
 *
 * @returns {Promise<string>} 完整 HTML 字符串
 */
async function renderFullHTML({ title, studentName, questions, showAnswers, qrContent }) {
 ...
 * @param {Array}  opt.questions       - 已选题目 JSON
 * @param {boolean} [opt.showAnswers=false]
 * @param {string} [opt.qrContent]
 * @param {string} [opt.filename]      - 下载文件名（不含 .pdf）
 * @returns {Promise<{downloaded: true, pdfBytes: number, filename: string}>}
 */
export async function exportServerPDF({ studentId, studentName, questions, showAnswers = false, qrContent, filename }) {
  if (!questions || questions.length === 0) {
    throw new Error('没有题目可生成 PDF')
  }
  ...
  const t0 = Date.now()
  console.log(`[serverPdfExporter] 开始渲染 HTML（${qs.length} 题）...`)

  // 1. 客户端跑 KaTeX + QR，得到完整 HTML
  const html = await renderFullHTML({
    title: filename || '错题重练',
  ...
  console.log(`[serverPdfExporter] HTML 渲染完成 ${Date.now() - t0}ms, ${(html.length / 1024).toFixed(1)}KB`)
  ...
  console.log(`[serverPdfExporter] PDF 下载完成 ${dt}ms, ${(blob.size / 1024).toFixed(1)}KB`)
