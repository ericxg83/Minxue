 ...
 * 
 * 渲染策略：
 * 1. 如果有 tikzCode（AI直接生成），优先使用 TikZ → SVG 转换
 * 2. 如果有 graphParams + graphType，使用参数生成 SVG
 * 3. 如果都没有，回退到局部图（src）
 * 4. 如果连局部图都没有，显示占位符
 */
 ...
 */
export function tikzToSvg(tikzCode, width = 380, height = 280) {
...
function functionParamsToSvg(params, width, height) {
  ...
  // 函数曲线（坐标点）
  if (params.coordinates && params.coordinates.length > 1) {
...
/**
 * 渲染图片区块：优先TikZ → SVG，回退局部图，最后占位符
 * @param {Object} block - 图片区块数据
 * @returns {{ type: 'svg'|'image'|'placeholder', content: string }}
 */
export function renderImageBlock(block) {
  // 1. 优先 TikZ 代码 → SVG
  if (block.tikzCode) {
    const svg = tikzToSvg(block.tikzCode)
    if (svg) {
  ...
  // 3. 回退到局部截图
  if (block.src) {
    return { type: 'image', content: block.src }
  }
