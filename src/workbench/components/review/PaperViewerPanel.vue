...
/**
 * 切题定位：保持当前 zoom（fitToContainer 或用户手动设置）不变，
 * 只调整 panX/panY，让 bbox 中心在容器中央可见。
 *
 * 设计原则：用户明确要求"我只要框换位置，我手动可以放大"。
 * 切题时自动改 zoom 会让图片跳来跳去、用户失去方向感，因此 zoom 由用户掌控。
 *
 * 算法：使用当前的 z（即 fitToContainer 的初始 zoom），计算让 bbox 中心对齐容器中心的 pan。
 *   - 图片比容器宽/高：自由调整 panX/panY 让 bbox 中心到中央；
 *   - 图片比容器窄/矮：图片居中（panX = (cw - imgW) / 2），不强行平移。
 *   - 边界夹紧到图片偏移范围，避免图片被推出容器。
 *   - 当 bbox 居中数学不可能（bbox 靠近图片边缘时，图片已无足够余量），
 *     退而求其次让 bbox 完整可见（在屏内），不强求精确中央。
 */
const jumpToBbox = (q) => {
  const bbox = getDisplayBox(q)
  ...
  // 保持 zoom 不变（fitToContainer 初始值 / 用户手动值）
  const z = zoom.value
  const imgW = nW * z
  const imgH = nH * z

  // X 轴：图片比容器宽 → 自由调整让 bbox 中心到容器中央；图片比容器窄 → 居中显示
  let newPanX
  if (imgW > cw) {
    const targetPanX = cw / 2 - bboxCenterX * z
    newPanX = Math.min(0, Math.max(cw - imgW, targetPanX))
  } else {
    newPanX = (cw - imgW) / 2
  }
  ...
  if (imgH > ch) {
    ...
    newPanY = Math.min(0, Math.max(ch - imgH, targetPanY))
  } else {
    newPanY = (ch - imgH) / 2
  }
