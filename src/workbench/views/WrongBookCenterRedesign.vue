          ...
          <div v-if="students.length" class="student-list" role="table" aria-label="所有学生">
            <div class="list-head" role="rowgroup">
              <div role="row" class="list-head__row" aria-hidden="true">
                ...
                <div role="columnheader">完全掌握</div>
      ...
      <ContentCard v-if="!embedded" class="student-overview" flush>
        <div class="student-profile">
          ...
          <div class="mastery-overview">
            ...
            <div class="mastery-meta"><span>共 {{ stats.total }} 道错题</span><span>{{ stats.mastered }} 道完全掌握</span></div>
          </div>
      ...
      <section class="management-workspace">
            ...
            <article v-for="item in paginatedQuestions" :key="item.id" :class="['question-record', { current: selectedQuestion?.id === item.id }]" @click="selectedQuestion = item">
              ...
              <div class="record-main">
                <div class="record-topline">
                  ...
                  <span v-if="subLabelOf(item)" class="sub-stem-label" :title="`原卷第 ${getQuestion(item).question_number} 题第 ${getQuestion(item).sub_no} 问`">{{ subLabelOf(item) }}</span>
        ...
        <ContentCard v-if="selectedQuestion" class="learning-record" flush>
          ...
          <div class="detail-scroll">
            <section class="detail-section question-preview"><label>题目内容</label><p v-if="parentStemOf(selectedQuestion)" class="parent-stem">{{ parentStemOf(selectedQuestion) }}</p><p>{{ contentOf(selectedQuestion) || '暂无题目文字内容' }}</p><img v-if="imageOf(selectedQuestion)" :src="imageOf(selectedQuestion)" alt="错题原图" /></section>
...
import { resolveQuestionDisplayStem, formatQuestionLabel } from '../../utils/questionStem'
