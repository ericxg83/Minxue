<template>
  <div class="retry-page review-task-page wb-page">
      ...
      <section class="retry-surface">
        ...
        <div v-else-if="visibleExams.length" class="retry-table">
          ...
          <article v-for="exam in visibleExams" :key="`${exam.student_id}-${exam.id}`" class="retry-row">
            ...
            <StatusTag :tone="statusType(exam.status)">{{ statusLabel(exam.status) }}</StatusTag>
...
<style scoped>
.retry-page { background: var(--wb-bg); }
...
.task-copy small, .muted, .student-name { color: var(--wb-text-secondary); font-size: 12px; } .row-action { min-width: 82px; }
