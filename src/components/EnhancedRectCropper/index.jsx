...
export default function EnhancedRectCropper({ 
  image, 
  onConfirm, 
  onCancel, 
  theme = 'light', 
  ...
  const [straightenedImage, setStraightenedImage] = useState(null)
  const [straightenProgress, setStraightenProgress] = useState(0)
  const [showStraightenOptions, setShowStraightenOptions] = useState(false)
  const [straightenOptions, setStraightenOptions] = useState({
    enableRotation: true,
  ...
  useEffect(() => {
    ...
    const loadImageAndStraighten = async () => {
      try {
        if (enableStraighten) {
          setStraightening(true)
          setStraightenProgress(10)
          
          // 检测偏转角度
          const angle = await detectImageSkewAngle(image)
          setStraightenProgress(30)
          
          // 应用拉直
          const straightened = await straightenImage(image, {
            ...straightenOptions,
            autoDetectAngle: true
          })
          setStraightenProgress(80)
          
          setStraightenedImage(straightened)
          setStraightenProgress(100)
          ...
          img.onload = () => {
            ...
            if (imgRef.current) {
              imgRef.current.src = straightened
            }
        ...
        console.error('图像加载/拉直失败，使用原图:', error)
        // 降级到原图
        const img = new Image()
        img.onload = () => {
          imgNaturalRef.current = { w: img.naturalWidth, h: img.naturalHeight }
    ...
    loadImageAndStraighten()
  }, [image, enableStraighten, straightenOptions])
      ...
      >
        ...
        <img
          ...
          style={{
            ...
            touchAction: 'none',
            pointerEvents: 'none',
            opacity: straightening ? 0.3 : 1
          }}
        />

        {imgRect.width > 0 && !straightening && (
          <>
            {showInstruction && (
              <div style={{
                position: 'absolute',
            ...
            {/* 拉直状态指示器 */}
            {enableStraighten && straightenedImage && (
              <div style={{
                position: 'absolute',
                top: 12,
                right: 12,
          ...
          <>
            ...
            {enableStraighten && (
              <div style={{ marginBottom: 12 }}>
                <button
                  onClick={() => setShowStraightenOptions(!showStraightenOptions)}
                  style={{
                    width: '100%',
                    padding: '10px',
                    borderRadius: 8,
                    border: `1px solid ${colors.accent}30`,
                    background: showStraightenOptions ? `${colors.accent}10` : 'transparent',
                    color: colors.accent,
                    fontSize: 13,
                    fontWeight: 500,
                    cursor: 'pointer',
                ...
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M21 2H3v20l9-6 9 6V2z"/>
                  </svg>
                  {showStraightenOptions ? '隐藏拉直选项' : '显示拉直设置'}
                </button>

                {showStraightenOptions && (
                  <div style={{
                    marginTop: 12,
                    padding: 12,
                    borderRadius: 8,
                    ...
                    <label style={{
                      ...
                      <input
                        type="checkbox"
                        checked={straightenOptions.enableRotation}
                        onChange={(e) => setStraightenOptions(prev => ({ ...prev, enableRotation: e.target.checked }))}
                    ...
                    <label style={{
                      ...
                      <input
                        type="checkbox"
                        checked={straightenOptions.enablePerspective}
                        onChange={(e) => setStraightenOptions(prev => ({ ...prev, enablePerspective: e.target.checked }))}
                        style={{ accentColor: colors.accent }}
                      />
                      启用透视校正
                    </label>
                    ...
                    <div style={{ marginBottom: 10 }}>
                      <div style={{ fontSize: 11, color: colors.textSecondary, marginBottom: 4 }}>
                        手动角度调整: {straightenOptions.manualAngle}°
                      </div>
                      <input
                        type="range"
                        min="-15"
                        max="15"
                        step="0.5"
                        value={straightenOptions.manualAngle}
                        onChange={(e) => setStraightenOptions(prev => ({ ...prev, manualAngle: Number(e.target.value) }))}
                  ...
                  <div style={{
                    ...
                    <div style={{ marginBottom: 10 }}>
                      <label style={{
                        ...
                        <input
                          type="checkbox"
                          checked={optimizationOptions.removeTextBg}
                          onChange={(e) => setOptimizationOptions(prev => ({ ...prev, removeTextBg: e.target.checked }))}
                          style={{ accentColor: colors.accent }}
                        />
                        去除文字背景（保留图形）
                      </label>
