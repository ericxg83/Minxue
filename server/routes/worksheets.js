...
export const ANSWER_OCR_SYSTEM_PROMPT = [
  ...
  '  · 「第十九章 单元测试卷」「期中评价测试卷」',
  '  · ⭐【试卷类小标题】必须原样输出，这是最容易漏识的一类：',
  '    - 「试卷① 19.1 平方根与立方根 基础性测试」',
  '    - 「试卷② 21.2(3) 一般的一元二次方程的解法」',
  '    - 「试卷 19.1(1)」「试卷一 19.1」「试卷1 算术平方根」',
  '    - 特征：行首出现"试卷"二字（带圈数字 ①..⑳ / 中文数字 一..十 / 阿拉伯数字），后面通常跟课时编号（19.1 / 21.2(3) 等）和小标题文字。',
  '    - 这类标题与"堂堂练① 19.1..."是同一层级的练习单元，题号从 1 重新开始，绝对不能漏！',
  '    - 漏掉它会导致"试卷①"下面所有题目错挂到上一个父章节（如"第十九章实数"），批改时整套试卷与答案库错位。',
