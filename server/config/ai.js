...
- "coordinate"（类型 A：坐标系 / 函数图）
  特征：图中【真实画出了】x 轴、y 轴、箭头、原点 O，或有函数曲线。
  → coordinate_system.exists = true，如实标出 x_axis / y_axis / origin。

- "geometry"（类型 B：纯几何示意图）
  特征：三角形、四边形、平行四边形、圆、辅助线、点 A/B/C 等，图中【没有】坐标轴。
  → coordinate_system.exists 必须 = false。绝对不要凭空添加坐标轴、刻度、原点。

- "geometry_with_coords"（类型 C：几何图带坐标背景）
  特征：以几何图形为主，但原图确实画有坐标轴作为背景。
  → 只保留原图【已经画出】的坐标轴，不要补全另一条不存在的轴。

判断硬性规则：只有当图中【肉眼可见地画出了带箭头的坐标轴线】时，figure_type 才能是 coordinate 或 geometry_with_coords；否则一律 geometry。宁可判成 geometry，也不要给几何题强加坐标系。

## 坐标轴方向约定（仅当确有坐标轴时）
标准数学方向：y 轴向上为正、x 轴向右为正，原点 O 在左下。
  y
  ↑
  |
  O ——→ x
识别原点位置时以此为准；不要把 y 轴方向判断成向下。
...
除此之外一律【禁止】自动添加：边长数字、坐标值、长度标记、角度数字、刻度。
中国初中教材插图默认风格：只有点、线、字母标记，没有多余数字、没有辅助说明。
...
9. 生成的 TikZ 代码应该可以直接编译生成与输入几何图形尽可能一致的矢量图。

【严格还原，不要重新设计】
10. 只复刻原图真实画出的元素，禁止根据题意或数学常识补充原图没有的点、线、辅助线、坐标轴或数字。

【坐标轴规则】（仅当原图确有坐标轴时才画）
11. 若原图没有坐标轴，绝对不要添加坐标轴、刻度或原点。
12. 若原图有坐标轴：x 轴向右为正、箭头在右端，x 标签放在右箭头旁（node[right] {$x$}）；y 轴向上为正、箭头在上端，y 标签放在上箭头旁（node[above] {$y$}）。绝不允许把 y 放在原点下方，也不要画向下的 y 轴或负方向轴标签（除非原图明确画出）。

【数字规则】
13. 只保留原图中真实画出的数字/长度/角度标注；禁止自动添加边长数字、坐标值、刻度、长度或角度数字。中国初中教材插图默认只有点、线、字母，没有多余数字。
