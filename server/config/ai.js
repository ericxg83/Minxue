...
- 必须与 block_coordinates 框选区域中实际显示的数字一致
...
【极其重要 - 坐标与内容匹配规则】
1. 每一个题目的 block_coordinates 必须是该题目文本在图片中的真实物理区域，绝不能张冠李戴！
2. 绝对禁止将上一题的坐标重复赋值给下一题，也绝对禁止出现多道题目的 y 坐标完全一致的低级错误！
3. 请按以下步骤自我校验：先找到试卷上"第 N 题"的文字位置，再确认 block_coordinates 框选的区域确实包围了该题的完整内容（题干+选项+作答区）
4. visual_title 字段必须与 block_coordinates 框选区域内实际显示的数字完全一致
5. 从上到下逐题扫描，确保每道题的 y 坐标都比上一题大（或至少在垂直方向上有明显区分），绝不能出现两道题 y 坐标相同的情况
