 ...
 *
 * 几何重建流程（第三阶段）：
 * 视觉模型只负责"看懂"几何图，输出点/线/圆/标注的结构化数据；
...
## 需要识别并保留的元素
1. 几何点（顶点）：字母标注 A/B/C/D/E/F/O/P/Q/M/N 等，估计其平面坐标
2. 几何线段：哪些点之间有连线，线型（实线/虚线/点划线）
3. 圆：圆心坐标、半径
4. 标注：
   - 长度标注（如 4、5cm、a、b、x 等，标在对应边的中点附近）
   - 角度标注（如 30°、45°、60°，标在对应角的顶点附近）
5. 直角标记：哪个顶点处是直角
...
3. 若图中没有圆，circles 用空数组 []；没有标注，labels 用空数组 []；没有直角，rightAngles 用空数组 []。
4. label 用图中实际出现的字母；segments 的 from/to 必须是 points 中存在的 label。
5. 如果裁剪图中没有任何有效几何结构（纯文字或无法识别），输出 {"points":[],"segments":[],"circles":[],"labels":[],"rightAngles":[]}。
6. 不要虚构原图中不存在的点、线、标注。
7. 如果裁剪图中意外出现了两个或多个几何图形（例如切图时带进了相邻题目的图），只识别【面积最大、最居中的那一个主图形】，忽略其余零碎图形和任何文字/题号。`
