...
2. style 只能是 "solid"、"dashed"、"dotted" 之一，无法判断时用 "solid"。
3. relation 只能是 "normal"、"perpendicular"、"parallel" 之一，无法判断时用 "normal"。
4. 若图中没有圆，circles 用空数组 []；没有几何标注，geometry_labels 用空数组 []；没有需忽略的数字，ignored_labels 用空数组 []；没有直角，rightAngles 用空数组 []；没有约束，constraints 用空数组 []。
5. 坐标轴存在性（coordinate_system.exists）必须严格根据图中是否有画出的坐标轴来判断，不要猜测。
6. label 用图中实际出现的字母；segments 的 from/to 必须是 points 中存在的 label。
7. 如果裁剪图中没有任何有效几何结构（纯文字或无法识别），输出 {"points":[],"segments":[],"circles":[],"geometry_labels":[],"ignored_labels":[],"rightAngles":[],"coordinate_system":{"exists":false,"origin":"","x_axis":false,"y_axis":false},"constraints":[]}。
8. 不要虚构原图中不存在的点、线、圆、标注。
9. 如果裁剪图中意外出现了两个或多个几何图形（例如切图时带进了相邻题目的图），只识别【面积最大、最居中的那一个主图形】，忽略其余零碎图形和任何文字/题号。
10. 坐标轴也是线段，但不要添加到 segments 中，只在 coordinate_system 中描述。`
...
4. 所有顶点字母标签（A/B/C/D 等）、角度标注、长度标注都必须与原图一致。
...
9. 尽量保持简洁，不添加原图中没有的辅助线或标注。
10. 生成的 TikZ 代码应该可以直接编译生成与输入几何图形尽可能一致的矢量图。
