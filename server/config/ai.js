...
- y: 题目区域左上角在图片中的纵坐标（像素）— **必须以题号（如"1."、"2."）的Y坐标为基准**
- width: 题目区域的宽度
- height: 题目区域的高度
- 请根据题目在图片中的实际位置进行标注

【极其重要 - 坐标标注规则】
1. **题号锚点定位（最高优先级）**：识别每一题时，必须先找到该题的阿拉伯数字题号（如"1."、"2."、"10."等）或中文题号（如"一、"、"二、"），以题号的Y坐标作为该题Y轴的绝对起点
2. **Y坐标必须递增**：每道题的 y 坐标必须比上一题大（从上到下逐题扫描，绝不出现两道题 y 坐标相同或倒序的情况）
3. **动态高度，严禁等分**：每道题目的高度（height）必须根据这道题实际包含的文字、手写步骤、函数图像的物理覆盖范围动态计算，严禁给出固定的、等分的高度数据！每道题的高度应该各不相同！
4. **【视觉定位死命令】**：你在计算每道题目的 block_coordinates 时，必须以题目的"阿拉伯数字题号"（如 21.、22.）作为该题的绝对顶边界（Y轴起点）。高亮框的顶部必须刚好压在题号上方 10 像素处，绝对不允许向下偏移吞掉下一题的文字！
5. **【容错外边距】**：为了防止漏字，每道题的顶部 y 坐标可以稍微往上提 15 像素（覆盖题号上方空白），底部 height 可以稍微多延伸 20 像素（确保包含最后一行作答内容）
6. **【严禁提取非题目区域】**：你绝对不能框选试卷的页眉、页脚、二维码、学生姓名、考试日期或单纯的标题（如"讯飞AI学"、"第X章"）。高亮框（block_coordinates）只能围绕真正的试题内容！
7. **【文字与坐标严格对应】**：你输出的每一个 question 对象，其 content（题目文字）必须与 block_coordinates（物理坐标框）完美重合！如果 content 是某道题的文字，坐标就必须紧紧包围那道题的区域，绝对不允许发生顺序错位或错位平移！
8. 先定位"第 N 题"的题号位置，再确认 block_coordinates 框选的区域包围了该题的完整内容（题干+选项+作答区）
9. 请按视觉顺序从上到下逐题识别，确保题目的顺序与试卷排版一致
10. 每个题目的 block_coordinates 必须是该题目在图片中的真实物理区域，绝不能张冠李戴
