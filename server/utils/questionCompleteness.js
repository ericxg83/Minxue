 ...
 * 引图关键词。
 *
 * [2026-09-21] `图1` → `图[0-9]+`：原正则只认「图1」，而卷面常见的「图2」「图8」
 * 「第3题图」全都匹配不到 → 这类题被判「不引图」，缺配图也不拦、错题本照收。
 * 实测第04周《28.2（2）》p2 第11~13 题的图注是「图8」形态，全部漏判。
 * 匹配任意数字不会放宽判据边界（仍然是「图」+ 数字，不误伤「图书」「图形」等词）。
 */
 ...
 *
 * 触发条件（四条同时满足，缺一不动）：
 *   1. question_type === 'choice'
 *   2. options 为空
 *   3. 题干没有内联选项标号，且不以空括号收尾（没有选择题证据）
 *   4. 题干含填空线
 * 其余情况一律原样返回 declared type（含非法/空值），纠偏范围刻意收窄到
 * 「选择题 + 零选项」这一种自相矛盾的组合。
 *
 ...
 * 题干（含公共题干）是否引用了图形。
 *
 * 与 checkQuestionCompleteness 规则1 同源，供「裁图 / 补裁 / 子题图形继承」共用，
 * 避免各处各写一份正则导致判据漂移（例如只查 content 而漏掉 parent_stem）。
 *
 * @param {Object} question
 * @returns {boolean}
 */
