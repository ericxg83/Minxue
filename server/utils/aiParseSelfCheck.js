...
/**
 * L1-c（2026-09-23 三层分流）：答案引擎空手而归时的「纯题干重解」输入口径。
 *
 * ── 为什么需要它 ──
 * 答案引擎偶尔**完全给不出答案**（返回空 / 「待人工补充」/「此为主观题，无唯一
 * 标准答案」）。实测近 14 天 `缺少参考答案` 110 道里 **95 道的 `answer` 是空的**，
 * 这些题本该有机会被自动补上，却只能转人工。
 *
 * ── 一个必须记住的测量教训（本条曾写错，2026-09-23 复测推翻）──
 * 本函数最初立项的依据是「110 道里 92 道（84%）的答案与学生答案逐字全等」，
 * 据此以为引擎在"抄学生笔迹"。复测证明**前提是假的**：
 *   · 真 `answer` 字段与学生答案全等 = **0/110**；`answer` 为空 = 95/110。
 *   · 被统计的字段其实是 `ai_answer`，而 worker.js 写的是
 *     `const aiAnswer = rawStudentAnswer` ⇒ **它按定义就等于学生答案**。
 * 拿它做相似度统计必然 100% 命中，属字段名义误读。且"引擎抄学生"这个场景
 * **早已由 P3 闸处理**（`isSameAnswerText` → 置空转后续链路）。
 * ⇒ 结论：本函数服务于"引擎失败后的二次机会"，与"抄学生"无关。
 *
 * ── 为什么不能用「掩掉笔迹重送图片」的方案 ──
