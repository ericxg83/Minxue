...
const SYMBOL_FOLD = [
  ...
  [/\\frac\s*\{([^{}]*)\}\s*\{([^{}]*)\}/gi, '($1)/($2)'],
  // LaTeX 角度：`45^\circ` / `45^{\circ}` → `45°`（2026-09-21 存量重跑实测补）
  [/\^?\s*\{?\s*\\circ\s*\}?/gi, '°'],
...
function stripScaffolding(s) {
  return String(s ?? '')
    .replace(/^[“"「『']\s*/, '').replace(/\s*[”"」』']$/, '')
    // LaTeX 行内/行间公式定界符：`$x=-\frac{1}{2}$` 与 `x=-1/2` 是同一个答案。
    // 2026-09-21 存量重跑实测：库内混着带 $ 与不带 $ 两种写法，不剥就白报分歧。
    .replace(/\$\$?/g, '')
    .replace(/^(?:最终答案|答案|正确答案|所以|因此|故|应为|应该是|应该|是|为|记作|写作)\s*[是为：:]?\s*/, '')
    .replace(/\s*(?:为准|即可|为止)\s*$/, '')
    // 2026-09-21 存量重跑实测补：库内答案常被写成「18√7，与答案一致」「B（已核对）」这种
    // 「答案 + 自证尾巴」的形式。不剥掉就会把同一个答案判成两个值，白报一次分歧。
    .replace(/\s*[，,（(]\s*(?:与答案一致|和答案一致|答案一致|与解析一致|与标准答案一致|已核对|核对无误|正确|无误)\s*[）)]?\s*$/, '')
    .trim()
}
